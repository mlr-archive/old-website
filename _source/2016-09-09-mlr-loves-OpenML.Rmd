...
At the time of writing this blog we are in Eindoven at an [OpenML
workshop](http://openml2016dev.openml.org/), where developers and scientists
meet to work on improving the project. Some of these people are R users and we
are developing an [R package](https://github.com/openml/openml-r) (which is
called OpenML) that connects to and communicates with the OpenML platform.

## OpenML in R
The OpenML R package can for example list and download data sets and machine
learning tasks (prediction challenges).  In R then one can run algorithms and
then upload the results to OpenML. The website then shows how well the
...
websit](http://www.openml.org/), in which we compare different types of Support
Vector Machines.  The study gets an ID assigned to it, which in our case is 25.
For this we use the function ksvm from package kernlab, which is itegrated in
mlr ("classif.ksvm").


For details on installing and setting up please see the
...
- Select all supervised classification tasks the do 10-fold cross-validation
  and choose only one task per data set. To make it fast, we focus on only the
first three tasks.
```{r}
...
of a certain study, tag it with study_ and the study ID.
```{r}
